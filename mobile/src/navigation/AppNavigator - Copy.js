...
function TabIcon({ label, focused }) {
  ...
  return (
    <View style={{ alignItems: 'center' }}>
      ...
      <Text style={{ fontSize: 10, color: focused ? colors.accent : colors.textMuted, marginTop: 2 }}>{label}</Text>
    </View>
  );
}
...
function MainTabs() {
  return (
    ...
    >
      <Tab.Screen name="Market" component={MarketScreen} options={{ tabBarIcon: ({ focused }) => <TabIcon label="Market" focused={focused} />, headerRight: () => <ProfileButton /> }} />
      <Tab.Screen name="Tastings" component={TastingsScreen} options={{ tabBarIcon: ({ focused }) => <TabIcon label="Tastings" focused={focused} /> }} />
      <Tab.Screen name="Collection" component={CollectionScreen} options={{ tabBarIcon: ({ focused }) => <TabIcon label="Collection" focused={focused} /> }} />
    </Tab.Navigator>
  );
}
...
function ProfileButton() {
  ...
  return (
    <Text onPress={() => navigation.navigate('Profile')} style={{ fontSize: 26, marginRight: 16 }}>{'\u{1F464}'}</Text>
  );
}

export default function AppNavigator() {
  const { isAuthenticated, isLoading, initialize, token, setUser } = useAuthStore();
  ...
  return (
    <NavigationContainer theme={{ dark: true, colors: { primary: colors.accent, background: colors.background, card: colors.surface, text: colors.text, border: colors.border, notification: colors.primary } }}>
